...
上面讲的都是图形界面。但很多场景其实不需要画布——比如想把一个流程接进 CI、写进脚本、或者在服务器上无头跑。所以 OpenWorkflows 也提供了一个命令行版本，对应的 skill 叫 `/openworkflows`。

设计上我刻意把它做得很克制：**在用户这一侧，命令只有两个**。因为在命令行里，你不需要关心蓝图、IRGraph、编译这些中间概念——**workflow 对你来说就是一个 `.js` 脚本**，其余的转换全部自动发生。

```bash
owf gen "做一个代码审查流程" -o review.js   # 用一句话生成一个 workflow 脚本
owf gen review.js "再加一个安全审查节点"      # 用一句话修改已有脚本
owf run review.js                          # 把脚本跑起来
```

就这三种用法（其实就 `gen` 和 `run` 两个命令）。

**`owf gen`** 是用自然语言生成或修改 workflow。它和界面底部那个 AI 输入框是同一套能力：你说需求，它生成脚本；你指着已有脚本说怎么改，它改给你。

这里有个关键点：**它是零配置的，不需要你去填什么 API Key**。因为它复用的就是你本机已经登录好的 `claude` CLI（和运行时用的是同一条路），所以只要你装了 claude 并登录过，`owf gen` 直接就能用。没装的话它会提醒你先 `claude login`。

**`owf run`** 就是把脚本跑起来，逐个节点执行，终端里实时打印进度：
